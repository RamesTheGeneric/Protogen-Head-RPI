...
class Idle():
    def mouth(ctx, mouth_x, mouth_y, eye_r_y, eye_l_y, surface, x_scale, y_scale):
        ...
        ctx.line_to(49 - (mouth_x[8] * x_scale), 22 - (mouth_y[8] * y_scale))
        ctx.line_to(42 - (mouth_x[7] * x_scale), 24 - (mouth_y[7] * y_scale))
        ctx.line_to(24 - (mouth_x[6] * x_scale), 17 - (mouth_y[6] * y_scale))
        ctx.line_to(24 - (mouth_x[6] * x_scale), 18 - (mouth_y[6] * y_scale))
        ctx.line_to(26 - (mouth_x[6] * x_scale), 19 - (mouth_y[6] * y_scale))
        ctx.line_to(42 - (mouth_x[5] * x_scale), 25 - (mouth_y[5] * y_scale))
        ctx.line_to(48 - (mouth_x[4] * x_scale), 23 - (mouth_y[4] * y_scale))
